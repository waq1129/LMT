function [BBwfun, BBwTfun, nu, sdiag] = prior_kernel_sp(rhoxx,lenxx,nt,latentTYPE,tgrid)
if nargin<5
    tgrid = [1:nt]';
end
...
switch latentTYPE
    ...
    case 2, % SE
        ...
        [B,sdiag] = covSEiso_sp(rhoxx,lenxx,tgrid);
        % Kprior = B'*diag(sdiag)*B;
        BBw = bsxfun(@times,B',sqrt(sdiag)');
        nu = length(sdiag);
        [u,s,v] = svd(BBw,'econ'); s = s.^2;
        sdiag = diag(s)+1e-4;
        iikeep = sdiag>sdiag(1)/1e6;

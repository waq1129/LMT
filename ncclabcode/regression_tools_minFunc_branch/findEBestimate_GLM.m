...
%wmap0 = minFunc(lfpost,w0,opts1); % do optimization for w_map
wmap0 = w0; % it's already at the MAP when I called this
...
opts2 = struct('maxFunEvals', 5, 'Method', 'newton', 'display', 'full');
hloss = @(ht)(updateMAPandEvalEvidence(ht,wmap0,opts2,mstruct)); % neg evidence func handle
%opts_fminunc = optimset('display', 'iter', 'largescale', 'off','maxfunevals',1e3, 'FinDiffType','central'); % opts governing evidence search
fprintf('Maximizing the evidence...\n');
opts_fminunc = struct('maxFunEvals', 30, 'method', 'lbfgs', 'display', 'full', 'numDiff', 1, 'optTol', 1e-2);
[htprsML,neglogEv1] = minFunc(hloss,htprs0,opts_fminunc); % maximize evidence

% get MAP estimate at these hyperparams
[neglogEv2,wEB,postHess] = updateMAPandEvalEvidence2(htprsML,wmap0,opts2,mstruct);
...
function [neglogEv] = updateMAPandEvalEvidence(htprs,wmap0,opts,mstruct)
...
wmap = minFunc(lfpost,wmap0,opts);

% evaluate negative log evidence using Laplace approxmiation
neglogEv = -logevid_GLM(wmap,hprs,mstruct);
% fprintf('update MAP and eval evidence: [%g][%g], [%g]\n', hprs(1), hprs(2), neglogEv);

function [neglogEv,wmap,postHess] = updateMAPandEvalEvidence2(htprs,wmap0,opts,mstruct)
...
neglogEv = -logevid_GLM(wmap,hprs,mstruct);
% fprintf('update MAP and eval evidence: [%g][%g], [%g]\n', hprs(1), hprs(2), neglogEv);
fprintf('update MAP and eval evidence: [%g], [%g]\n', hprs(1), neglogEv);

% test basic Poisson ridge-regression code on 1D simulated example
...
fnlin = @expfun;  % nonlinearity (must return func and 1st & 2nd derivs)
...
tt = 1:nw;
subplot(212);
plot(tt,wts,'k'); title('true filter');
subplot(211);
xpl = min(xproj):.1:max(xproj);
plot(xproj,yy,'.',xpl,fnlin(xpl), 'k');
...
%% 3. Compute Empirical Bayes Poisson-regression estimate, ridge prior
...
[wRidge,rhoHat,SDerrbars,Hess] = autoRegress_PoissonRidge(xx,yy,fnlin,rdgInds,rhoNull,rhovals,wmap0);
...
rhoNull = .01;  % prior precision for other variables
[wAR1,hprsAR1,SDerrbarsAR1,HessAR1] = autoRegress_PoissonAR1(xx,yy,fnlin,nw,rhoNull,rhovals,avals,wmap0);

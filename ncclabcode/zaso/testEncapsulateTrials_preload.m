...
X{1} = [1 2 3];
Y{1} = [1];
zaso = encapsulateTrials_preload(X, Y);
...
assert(all(zasoFx(zaso, @(x) sum(x, 2)) == cellfun(@sum, X)));
assert(all(zasoFy(zaso, @(x) sum(x, 2)) == cellfun(@sum, Y)));
[rsum, ragg] = zasoFarray(zaso, {@(x,y) x' * x, @(x,y) x' * y}, {@(x,y) sum(x,2)});
...
zaso = encapsulateTrials_preload(X, Y, tbX);
...
zaso = encapsulateTrials_preload(X, Y, tbX);
assert(zaso.Nsub == 8);
assert(all(zaso.sub2idx(1) == 1));
assert(all(zaso.sub2idx(2) == 2));

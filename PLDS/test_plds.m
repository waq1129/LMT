...
for dayid = 1:10;
    for epochid = [2,4];
        for ii=1:20
            nneurname = ['../crcns/hc6/neuron5_' an '_d' num2str(dayid) '_e' num2str(epochid) '_s' num2str(ii) '.mat'];
            if exist(nneurname, 'file')~=2
                continue;
            end
            nneurname
            load(nneurname)
            ...
            try
                ...
                %pause
                save(['neuron5_' an '_d' num2str(dayid) '_e' num2str(epochid) '_s' num2str(ii) '_plds'],'x11')
            catch
                continue;
            end
        end
